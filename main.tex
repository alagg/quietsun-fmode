...
%MJK Starting completely anew
%MJK Not only errors
%MJK Now I am assuming that the yearly variation has been removed, and from here onwards, we are dealing with the cleaned data.
%Error 
Definitions:\\
$\lambda$ = latitude, $t$=time, \\
$N_t$ = number of bins 
%MJK avoiding brackets
in time \\
%(time), \\
$N_\lambda$ = number of bins 
%MJK Avoiding brackets
%(latitude), \\
in latitude, \\
%MJK Added, needed for the proper definition of fprime
...
%MJK added "without binning"
without binning,\\
$f(\lambda,t)$ = $f$-mode strength \\
%MJK Unnecessary
%(unbinned)\\
$f_P=1/N_P \sum_{N_P} f (\lambda,t)$ = mean $f$-mode strength 
%in latitude / time bins (patches)\\
%MJK Reform
calculated in patches of $N_t$ and $N_\lambda$ \\
%MJK N_t is incorrect here N_t -> n_t
...
%MJK Added
The root-mean-square value of this quantity is used from calculating the amplitude of the solar cycle variation, and is defined as $f'_{\rm rms} (\lambda) = \sqrt{\overline{f'^2}}$.
%MJK Unnecessary
%(unbinned)\\
%MJK Where is this even needed? This quantity should not be used anywhere. For computation of fprime we use
%MJK \overline{f}. I think this can be removed.
...
- f_P)^2}.
\end{equation}
From this we form the average error at each latitude by taking an average over time. 
%MJK NEW STUFF ENDS HERE
