...
%MJK Nishant16 called that f-mode energy, and used E_f for it.
...
%MJK
to obtain $P_{\omega,k}$. An illustrative case for a frequency
value at which f-mode is strong, is shown in 
%(see 
\fig{ring_diagram},
%MJK
upper panel.
...
in this kind of a plot, the f-mode is the rightmost peak at the highest $k$ values.
%MJK
%Such 
The
averaging 
...
%From the obtained spectra $f$-mode was
%easily separable from the rest of the modes and integration done over $k$ and $\omega$, to obtain full power of the $f$-mode.
To obtain the total energy contained in the f-mode, $E_f$, we
performed another integration over the separated f-mode signal.
In contrast to earlier studies \citet{SRB16,Waidele22}, we
did not perform fitting to the f-mode for its extraction. 
...
%MJK
where the significant part of the $f$-mode power resides. 
%MJK 
As the end result, we have computed the f-mode energy
...
environment, we had collected a total of \todo{18327 number to be updated for final version!} $f$-mode 
%powers 
energies
