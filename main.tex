...
\begin{itemize}
\item sdev(cycle) $\sigma(\lambda)$ (red shaded area): this quantity displays the variation of the $f$-mode over the solar cycle
\begin{equation}
%MJK I think subscript P is missing from here, and the need for 'cyc' I cannot understand.
%\sigma^2_{\mbox{cyc}}(\lambda) 
\sigma_P^2(\lambda)  = \frac{1}{N_t} \sum_{t} \left( f_P(\lambda,t) -\overline{f_P}(\lambda)\right)^2,
\end{equation}
%MJK added this
...
\item mean(sdev(patch)) $\overline{\sigma_P}$ (error bars): the standard deviation of the $f$-mode strength is computed individually for all lat / time bins ($=\sigma_P(\lambda,t)$) and then averaged over time:
\begin{equation}
\overline{\sigma_P}(\lambda) =  \frac{1}{N_t} \sum_{t}  \sigma_P(\lambda,t).
\end{equation} 
%MJK Added this
