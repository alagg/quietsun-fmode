...
The detailed data pipeline is summarized in \fig{pipeline}.
...
The significance of the results was verified by rejecting a null hypothesis in a following way: For all quiet sun patches we computed the power of noise in the azimuthally averaged spectra similarly as for $f$-modes. We assumed that noise level is independent of $k$ and selected a region towards higher $k$ values, where the $f$-mode is already decayed. The analysis showed that the noise power was an order of magnitude weaker than the corresponding $f$-mode powers.
