...
about
the magnetic field in the quiet Sun regions and for a robust computation of the \fff power from the dopplergrams. The left tree in \fig{pipeline} describes the pipeline used for the magnetograms, the right tree for the dopplergrams.
%MJK
...
%MJK We neglected differential rotation due to
...
\colfigtwocol{2010-07-21_2014-11-09}{Comparison of two disk-center magnetograms at solar minimum (2010-07-21) and solar maximum 
...
%$N_P$ = number of \ef measurements in a patch, \\
...
%MJK This is not needed; f==E_f
%\%$f(\lambda,t)$ = \fff energy, \ef, as function of latitude and time \\
%AL removed _P
%$\left<f_P\right>=1/N_P \sum_{N_P} f (\lambda,t)$ = mean \fff strength 
%$\left< f \right>=1/N_P \sum_{N_P} f (\lambda,t)$ = mean \fff strength calculated in patches of $N_t$ and $N_\lambda$ \\
%$\left<\mef \right>=1/N_P \sum_{N_P} \mef (\lambda,t)$ = mean \fff strength calculated in patches of $N_t$ and $N_\lambda$ \\
%$\overline{f}(\lambda) = 1/n_t \sum_t f(\lambda,t)$ = temporal mean of the \fff strength, \\
%$\overline{\mef}(\lambda) = 1/n_t \sum_t \mef(\lambda,t)$ = temporal mean of the \fff strength, \\
...
%$\mef'=\mef-\overline{\mef}$ is the fluctuation around the temporal mean, i.e. the solar cycle dependence of the \fff.\\
%AL: In the butterfly we plot the binned f'. It is computed like this:
%$\left< \mef' \right> = \left< \mef - \overline{\mef} \right> $ is the same as $\mef'$, but computed in $N_\lambda \times N_P$ bins. This is the quantity we show in the left panel of \fig{butterfly_lat}.
%MJK Added
...
%${\mef'}_{\rm A} (\lambda) = 
%\sqrt{2} \sqrt{\left<{\mef'^2}\right>}$. 
%The average over the bin is necessary to remove the noise from the estimate.
%AL added
%This quantity ${\mef'}_{\rm A} (\lambda)$ is plotted as the orange shaded area in the right panel of \fig{butterfly_lat}.

%One measure of the 
%noise contained in the \fff strength is the standard deviation of the fluctuations
...
%MJK Maybe we do not need this anymore
...
%MJK We also need to show the scatterplot of Brms vs. \fff per year.
...
%MJK Have no idea whether this is true, but just writing this as a placeholder.
...
The results from our analysis are shown with orange symbols and lines in \Fig{AR11130_track} and \Fig{AR11105_track}: each orange point shows the normalized \fff energy, \tef, with four-hour cadence, smoothed with XXXX over one day to remove a daily fluctuation otherwise prominently present in the data. The grey-shaded areas show the QS variance, $E'_f$, which is roughly constant, around 4--5 percent, at all latitudes and longitudes. When it is used as normalization of the \fff, then the uncertainty at large longitudes becomes very high, as is indicated by the rapidly expanding grey-shaded areas toward the limb.
...
As can be seen
from \Fig{AR11130_track} and \Fig{AR11105_track}, the effect of the new QS calibration with Gaussian smoothing is
...
%MJK Cannot remember enough of the AR 11105 to write something about it now.
...
\colfig{fmode_vs_brms}{\fff strength as a function of \brms{} for the most quiet patches at disk center. The colors indicate the grouping according to the years. The standard deviation for the \fff variation is 87.}


%AL adding two AR plots as placeholders. Needs to be discussed what should be shown
\colfig{AR11130_track}{\fff strength (top) and $B_{\mbox{rms}}$ (bottom) for AR11130. Orange: quiet-Sun calibration, blue: cosine calibration.}
